...
export class AdminSellerController {
  constructor(private readonly sellerService:SellerService) {}
        ...
        @Get('Seller_search')
        @Roles(Role.L2A_ADMIN)
        async search(@Query('page') page: number = 1,@Query('pageSize') pageSize: number =10,@Query('query') query?: string, @Query('categoryId') categoryId?: string, @Query('type') type?: string, @Query('country') country?: string, @Query('city') city?: string, @Query('state') state?: string)
        {
            return this.sellerService.search(page ,pageSize,query, categoryId,type,country,city,state);
        }
