...
export class SellerService {
  constructor(
       ...
       async search(pageNumber: number,pageSize?:number, query?: string, categoryId?: string,type?: string,country?: string,city?: string,state?: string)
       {

           const skip = (pageNumber - 1) * pageSize;
           const [result, totalCount] = await this.SellerRepository.search(skip,pageSize,query,categoryId,type,country,city,state);
           const totalPages = Math.ceil(totalCount / pageSize);
           if (result.length === 0)
           {
              throw new NotFoundException('No records found');
