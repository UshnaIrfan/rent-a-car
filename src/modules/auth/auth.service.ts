...
export class AuthService {
  ...
  async getAllUsers(
    ...
    pageSize?: number,
    username?: string,
  ): Promise<paginationUserInterface> {
  ...
  async adminUpdateUserStatus(
    adminUpdateStatus: adminUpdateUserInterface,
  ): Promise<{ update: User; message: string }> {
    const update = await this.usersService.adminUpdateUserStatus(
      adminUpdateStatus.userId,
  ...
  async adminUpdateUserBlockStatus(
    adminUpdateBlockStatus: adminUpdateBlockStatusUserDto,
  ): Promise<{ update: User; message: string }> {
  ...
  async signup(@Body() Signup: signupUserInterface) {
    ...
    if (User && User.status == 'inactive') {
      ...
      throw new ConflictException(
        'You are already created this account. Resend email please verify your account.',
      );
    } else {
      const username = await this.usersService.findUserByUsername(
        Signup.username,
      );
      if (username) {
  ...
  async token(randomUserToken: randomUserTokenInterface) {
    ...
    const template = handlebars.compile(
      fs.readFileSync('src/templates/resetPassword.html', 'utf8'),
    );
  ...
  async Password(@Body() reqBody: changeUserPasswordInterface) {
    ...
    if (parsedToken.active == true) {
      ...
      throw new UnauthorizedException(
        'plz firstly active token and then  password changed',
      );
    }
  }
  ...
  async sendWelcomeEmail(email: string, Username: string) {
    const template = handlebars.compile(
      fs.readFileSync('src/templates/welcomeEmail.html', 'utf8'),
    );
  ...
  async sendVerificationEmail(email: string, emailBody: string) {
    await this.mailerService.sendMail({
      to: email,
      subject: 'Please confirm you recent signup!',
      html: emailBody,
    });
  }
